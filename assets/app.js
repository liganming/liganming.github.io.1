...
var homepageVideo = {
    ...
    createFrame: function(settings, videoDetails) {
        ...
        if (videoDetails.provider === 'youtube') {
            ...
            var html = '<video autoplay="true" ' + autoloop + ' id="video" onclick="this.paused?this.play():this.pause();"><source src="' + videoDetails.videoMP4 + '" type="video/mp4"><source src="' + videoDetails.videoOGV + '" type="video/ogg" ></video>';
        }
        return html;
    },
    appendFrame: function(settings, videoDetails) {
        var addVideo = function() {
            ...
            settings.container.find('iframe').remove();
            settings.container.find('video').remove();
            settings.container.append(homepageVideo.createFrame(settings, videoDetails));
            settings.container.addClass('video-added');

            if (videoDetails.provider != 'html5' && settings.container.find('.mediaWrapper').length < 1) {

                var paddingBottomValue = videoDetails.aspectRatio[1] / videoDetails.aspectRatio[0] * 100 + '%';
                ...
                settings.container.find('.hsContainer__image').css({
                    paddingBottom: paddingBottomValue
                });

            } else if (settings.container.find('#video').length < 1) {
...
var map = {
    init: function() {
        if ($(".map-section .map, .contact-template-section .map, .page-details-section .map").hasClass('js-api-map')) {
            var mapsToLoad = [];
    ...
    if ($cartBtn && $cartBtn.length) {
        ...
        if (cart.item_count == 0) {
            ...
            $.each(cart.items, function(index, item) {
                var line_id = index + 1;
                cart_items_html += '<li class="cart_item">' + '<p class="mm-counter">' + '<span class="icon-minus minus"></span>' + '<input type="number" min="0" class="quantity" name="updates[]" id="updates_' + item.id + '" value="' + item.quantity + '"  data-line-id="' + line_id + '" readonly />' + '<span class="icon-plus plus"></span>' + '</p>' + '<a href="' + item.url + '">';
                if (item.image) {
                    cart_items_html += '<div class="cart_image">' + '<img src="' + item.image.replace(/(\.[^.]*)$/, "_medium$1").replace('http:', '') + '" alt="' + htmlEncode(item.title) + '" />' + '</div>';
                }
            ...
            cart_items_html += '<li class="mm-label">' + '<p class="mm-counter price"><span class="money">' + Shopify.formatMoney(cart.total_price, $('body').data('money-format')) + '</span></p>' + '<a href="/cart">' + '<strong>Subtotal</strong>' + '</a>' + '</li>' + '<li class="mm-subtitle clearfix">';

            cart_items_html += '<input type="submit" class="action_button right" value="Checkout" />' + '<a href="/cart" class="action_button edit_cart right">View Cart</a>' + '</li>';
...
$(function() {
    ...
    // Sync currencies
    if ($(window).width() > 798 || !/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        $("#nav select.currencies").val($('.header select.currencies').find(":selected").text());
        $('.header select.currencies').change(function() {
            $("#nav select.currencies").val($(this).find(":selected").text());
        });
    }

    if ($('.shopify-section.page-details-section').length) {// Don't apply mediaWrapper
...
//  });
    ...
    $('body').on('submit', '.product_form form', function(e) {
        ...
        var $addToCartInfo = $(this).parents('.product_form');
...
Shopify.newState = function(t, e) {
    var o;
    return o = Shopify.urlParam(t) ? Shopify.locationSearch().replace(RegExp("(" + t + "=)[^&#]+"), "$1" + e) : "" === Shopify.locationSearch() ? "?" + t + "=" + e : Shopify.locationSearch() + "&" + t + "=" + e,
...
Shopify.OptionSelectorsFromDOM.prototype.createProductFromSelector = function(t, e, o, i) {
    if (!Shopify.isDefined(o))
        var o = !0;
    if (!Shopify.isDefined(i))
        var i = "/";
    ...
    var a = function() {
        ...
        return function(b, e) {
            ...
            f = e.match(/^mailto:([^\/].+)/))
                ...
                if ((f = e.match(/(.*?)\/#\!(.*)/)) && (e = f[1] + f[2]),
                (f = e.match(/(.*?)#(.*)/)) && (g.hash = f[2],
                e = f[1]),
                g.hash && b.match(/^#/))
                    return d(b, g.hash);
...
!function(t) {
    t.fn.mediaWrapper = function(i) {
        ...
        return this.each(function() {
            var i = "";
            i = 1 == e.intrinsic && "" !== t(this).attr("width") && "" !== t(this).attr("height") ? t(this).attr("height") / t(this).attr("width") * 100 : e.baseHeight / e.baseWidth * 100,
            t(this).wrap('<div class="mediaWrapper" style="position: relative; width: 100%; height: 0; padding: ' + i + '% 0 0 0; " />').css({
                position: "absolute",
                width: "100%",
                height: "100%",
                top: "0",
    ...
    f.extend(b, {
        ...
        defaults: {
            ...
            tpl: {
                ...
                next: '<a title="Next" class="fancybox-nav fancybox-next ss-icon" href="javascript:;"><span class="icon-circle-right"></span></a>',
                prev: '<a title="Previous" class="fancybox-nav fancybox-prev ss-icon" href="javascript:;"><span class="icon-circle-left"></span></a>'
            },
            openEffect: "fade",
            openSpeed: 250,
            openEasing: "swing",
...
!function($) {
    $.flexslider = function(e, t) {
        ...
        m = {
            ...
            directionNav: {
                setup: function() {
                    var e = $('<ul class="' + n + 'direction-nav"><li class="' + n + 'nav-prev"><a class="' + n + 'prev ss-icon" href="#"><span class="icon-circle-left"></span></a></li><li class="' + n + 'nav-next"><a class="' + n + 'next ss-icon" href="#"><span class="icon-circle-right"></span></a></li></ul>');
            ...
            touch: function() {
                ...
                function n(t) {
                    ...
                    if (a) {
                        ...
                        t.detail === t.MSGESTURE_FLAG_INERTIA ? void setImmediate(function() {
                            e._gesture.stop()
                        }) : void ((!y || Number(new Date) - f > 500) && (t.preventDefault(),
                        !v && a.transitions && (a.vars.animationLoop || (m = w / (0 === a.currentSlide && 0 > w || a.currentSlide === a.last && w > 0 ? Math.abs(w) / p + 2 : 1)),
                ...
                h = function(e) {
                    ...
                    !v && a.transitions && (a.vars.animationLoop || (m /= 0 === a.currentSlide && 0 > m || a.currentSlide === a.last && m > 0 ? Math.abs(m) / p + 2 : 1),
                    a.setProps(l + m, "setTouch")))
                }
                ,
                S = function(t) {
                    if (e.removeEventListener("touchmove", h, !1),
            ...
            "undefined" != typeof s) {
                ...
                if (s.length) {
                    n.each(s, function(i) {
                        ...
                        "object" != typeof l && (l = {}),
                        "undefined" == typeof l.content && (l.content = ["prev", "title"]),
                        l.content instanceof Array || (l.content = [l.content]),
                        l = n.extend(!0, {}, r.opts.navbar, l);
                        var d = l.position
                          , h = l.height;
                        "number" != typeof h && (h = 1),
                        ...
                        o[d] || (o[d] = 0),
                        o[d]++;
                        var f = n("<div />").addClass(e.navbar + " " + e.navbar + "-" + d + " " + e.navbar + "-" + d + "-" + o[d] + " " + e.navbar + "-size-" + h);
                        o[d] += h - 1;
                        for (var v = 0, u = 0, p = l.content.length; u < p; u++) {
                            var b = n[a].addons[t][l.content[u]] || !1;
                            b ? v += b.call(r, f, l, c) : (b = l.content[u],
                            b instanceof n || (b = n(l.content[u])),
                            f.append(b))
                        }
                        v += Math.ceil(f.children().not("." + e.btn).length / h),
                        v > 1 && f.addClass(e.navbar + "-content-" + v),
                        f.children("." + e.btn).length && f.addClass(e.hasbtns),
                        f.prependTo(r.$menu)
                    });
                    for (var l in o)
                        r.$menu.addClass(e.hasnavbar + "-" + l + "-" + o[l])
                }
            }
        },
        add: function() {
...
function(n) {
    ...
    n[a].addons[t][e] = function(t, e, r) {
        ...
        var c = n('<span class="' + s.breadcrumbs + '" />').appendTo(t);
        this.bind("initPanels", function(a) {
            a.removeClass(s.hasnavbar).each(function() {
                for (var a = [], t = n(this), e = n('<span class="' + s.breadcrumbs + '"></span>'), c = n(this).children().first(), o = !0; c && c.length; ) {
                    c.is("." + s.panel) || (c = c.closest("." + s.panel));
                    var l = c.children("." + s.navbar).children("." + s.title).text();
                    a.unshift(o ? "<span>" + l + "</span>" : '<a href="#' + c.attr("id") + '">' + l + "</a>"),
                    o = !1,
                    c = c.data(i.parent)
                }
        ...
        var o = function() {
            c.html(this.$pnls.children("." + s.current).children("." + s.navbar).children("." + s.breadcrumbs).html())
        };
        return this.bind("openPanel", o),
        this.bind("initPanels", o),
        0
    }
}(jQuery),
/*
...
function(n) {
    var a = "mmenu"
      , t = "navbars"
      , e = "close";
    n[a].addons[t][e] = function(t, e) {
        var r = n[a]._c
          , s = n[a].glbl
          , i = n('<a class="' + r.close + " " + r.btn + '" href="#" />').appendTo(t)
          , c = function(n) {
            i.attr("href", "#" + n.attr("id"))
        };
        return c.call(this, s.$page),
        this.bind("setPage", c),
        -1
    }
}(jQuery),
...
function(n) {
    ...
    n[a].addons[t][e] = function(e, r) {
        var s, i, c, o = n[a]._c, l = n('<a class="' + o.next + " " + o.btn + '" href="#" />').appendTo(e), d = function(n) {
            n = n || this.$pnls.children("." + o.current);
            var a = n.find("." + this.conf.classNames[t].panelNext);
            s = a.attr("href"),
            c = a.attr("aria-owns"),
            i = a.html(),
            l[s ? "attr" : "removeAttr"]("href", s),
            l[c ? "attr" : "removeAttr"]("aria-owns", c),
            l[s || i ? "removeClass" : "addClass"](o.hidden),
            l.html(i)
        };
    ...
    ,
    n[a].configuration.classNames[t].panelNext = "Next"
}(jQuery),
/*
...
function(n) {
    ...
    n[a].addons[t][e] = function(e, r) {
        var s = n[a]._c
          , i = n('<a class="' + s.prev + " " + s.btn + '" href="#" />').appendTo(e);
        this.bind("initPanels", function(n) {
            n.removeClass(s.hasnavbar).children("." + s.navbar).addClass(s.hidden)
        });
        var c, o, l, d = function(n) {
            if (n = n || this.$pnls.children("." + s.current),
            !n.hasClass(s.vertical)) {
                var a = n.find("." + this.conf.classNames[t].panelPrev);
                a.length || (a = n.children("." + s.navbar).children("." + s.prev)),
                c = a.attr("href"),
                l = a.attr("aria-owns"),
                o = a.html(),
                i[c ? "attr" : "removeAttr"]("href", c),
                i[l ? "attr" : "removeAttr"]("aria-owns", l),
                i[c || o ? "removeClass" : "addClass"](s.hidden),
                i.html(o)
            }
        };
        return this.bind("openPanel", d),
    ...
    ,
    n[a].configuration.classNames[t].panelPrev = "Prev"
}(jQuery),
/*
...
function(n) {
    ...
    n[a].addons[t][e] = function(t, e) {
        var r = n[a]._c
          , s = n('<div class="' + r.search + '" />').appendTo(t);
...
function(n) {
    var a = "mmenu"
      ...
      , e = "title";
    n[a].addons[t][e] = function(e, r) {
        var s, i, c = n[a]._c, o = n('<a class="' + c.title + '" />').appendTo(e), l = function(n) {
            if (n = n || this.$pnls.children("." + c.current),
            !n.hasClass(c.vertical)) {
                var a = n.find("." + this.conf.classNames[t].panelTitle);
                a.length || (a = n.children("." + c.navbar).children("." + c.title)),
                s = a.attr("href"),
                i = a.html() || r.title,
                o[s ? "attr" : "removeAttr"]("href", s),
                o[s || i ? "removeClass" : "addClass"](c.hidden),
                o.html(i)
            }
        };
    ...
    ,
    n[a].configuration.classNames[t].panelTitle = "Title"
}(jQuery);
...
!function(i) {
    ...
    i[t].addons[n] = {
        setup: function() {
            ...
            this.bind("openPanel:start", function(i) {
                var t = this.$menu.find("." + e.panel).not(i).not(i.parents("." + e.panel))
                  , n = i.add(i.find("." + e.vertical + "." + e.opened).children("." + e.panel));
                this.__sr_aria(t, "hidden", !0),
                this.__sr_aria(n, "hidden", !1)
            }),
            this.bind("closePanel", function(i) {
                this.__sr_aria(i, "hidden", !0)
            }),
            this.bind("initPanels:after", function(t) {
                var n = t.find("." + e.prev + ", ." + e.next).each(function() {
                    a.__sr_aria(i(this), "owns", i(this).attr("href").replace("#", ""))
                });
                this.__sr_aria(n, "haspopup", !0)
            }),
            this.bind("initNavbar:after", function(i) {
                var t = i.children("." + e.navbar);
                this.__sr_aria(t, "hidden", !i.hasClass(e.hasnavbar))
            }),
            h.text && (this.bind("initlistview:after", function(i) {
                var t = i.find("." + e.listview).find("." + e.fullsubopen).parent().children("span");
                this.__sr_aria(t, "hidden", !0)
            }),
            "parent" == this.opts.navbar.titleLink && this.bind("initNavbar:after", function(i) {
                var t = i.children("." + e.navbar)
                  , n = !!t.children("." + e.prev).length;
                this.__sr_aria(t.children("." + e.title), "hidden", n)
            }))),
            ...
            this.bind("initNavbar:after", function(n) {
                var r = n.children("." + e.navbar)
                  ...
                  , s = i[t].i18n(o.text.closeSubmenu);
                a && (s += " (" + a + ")"),
                r.children("." + e.prev).html(this.__sr_text(s))
            }),
            this.bind("initListview:after", function(n) {
                var s = n.data(r.parent);
                if (s && s.length) {
                    var h = s.children("." + e.next)
                      , d = h.nextAll("span, a").first().text()
                      , f = i[t].i18n(o.text[h.parent().is("." + e.vertical) ? "toggleSubmenu" : "openSubmenu"]);
                    d && (f += " (" + d + ")"),
                    h.html(a.__sr_text(f))
                }
            }))
        },
        add: function() {
    ...
    i[t].prototype.__sr_aria = function(i, t, n) {
        i.prop("aria-" + t, n)[n ? "attr" : "removeAttr"]("aria-" + t, n)
    }
    ,
    i[t].prototype.__sr_text = function(i) {
        return '<span class="' + e.sronly + '">' + i + "</span>"
    }
    ;
    var e, r, a, s
}(jQuery);
...
!function(e, n, t) {
    ...
    function a(e) {
        ...
        p ? c.className.baseVal = n : c.className = n)
    }
    function i() {
    ...
    function l(e, t, o, s) {
        var a, l, f, d, u = "modernizr", p = i("div"), m = r();
        if (parseInt(o, 10))
            for (; o--; )
                f = i("div"),
                f.id = s ? s[o] : u + (o + 1),
        ...
        var d = /\s+/g
          ...
          , k = function(a, c, g) {
            ...
            a.forEach(function(a) {
                ...
                a.match(f) && (c.setAttribute(lazySizesConfig.srcsetAttr, RegExp.$1),
                RegExp.$2 && c.setAttribute("media", lazySizesConfig.customMedia[RegExp.$2] || RegExp.$2)),
                h.appendChild(c)
            }),
          ...
          , l = function(a) {
            if (a.target._lazybgset) {
                ...
                e && (d.style.backgroundImage = "url(" + (g.test(e) ? JSON.stringify(e) : e) + ")"),
        ...
        a.addEventListener("lazybeforesizes", function(a) {
            if (a.detail.instance == c && a.target._lazybgset && a.detail.dataAttr) {
                var b = a.target._lazybgset
                  , d = j(b);
                h[d] && (a.target._lazysizesParentFit = d,
                c.rAF(function() {
...
var destroySlider = function(slider) {
    ...
    if (slider && slider.data('flexslider')) {
        ...
        if (slider.vars.controlNav)
            slider.controlNav.closest(classNamespace + 'control-nav').remove();
        // Remove control elements if present
        if (slider.vars.directionNav)
            slider.directionNav.closest(classNamespace + 'direction-nav').remove();
        // Remove direction-nav elements if present
        if (slider.vars.pausePlay)
            slider.pausePlay.closest(classNamespace + 'pauseplay').remove();
...
$(document).on('shopify:section:load', function(e) {
    ...
    if ($parentSection.hasClass('map-section') || $parentSection.hasClass('contact-template-section') || $parentSection.hasClass('page-details-section')) {
        map.init();
    }
